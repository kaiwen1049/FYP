...
if (isset($_POST['signup_btn'])) {
	...
	if (count($errors) === 0) {
		...
		$query = "INSERT INTO users SET role_id=?, username=?, email=?, password=?, created_at=?";
		$stmt = $conn->prepare($query);
		$stmt->bind_param('issss', $role_id, $username, $email, $password, $created_at);
		$result = $stmt->execute();
		if ($result) {
...
if (isset($_POST['signup_admin'])) {
	// validate form values
	$errors = validateUser($_POST, ['signup_admin']);
